...
parser.add_argument(
    "--conf_dir",
    default="/home/kaloori.shiva/Thesis/TIGER/Experiments/checkpoint/TIGER-Librimix/conf.yml",
...
def main(config, checkpoint, test_subset):
    ...
    with progress:
        for idx in progress.track(range(test_size)):
            ...
            # Around line 114 in audio_test_new.py
            # print(f"Testing sample index {idx}: {key}") # Add this to see the filename
            try:
                metrics(mix=mix_np, clean=sources_np, estimate=est_sources_np, key=key)
            except Exception as e:
                print(f"Error at index {idx}, key {key}: {e}")
                print(f"Estimate shape: {est_sources_np.shape}, Clean shape: {sources_np.shape}")
                # Optional: save the problematic tensors to inspect later
                # np.save("error_est.npy", est_sources_np) 
                continue
...
# python audio_test_new.py --conf_dir /home/kaloori.shiva/Thesis/TIGER/configs/tiger-small.ymlopenSLR_hin_dataset/kathbath_noise