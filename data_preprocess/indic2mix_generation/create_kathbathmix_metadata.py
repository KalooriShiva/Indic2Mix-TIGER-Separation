# create_kathbathmix_metadata.py
import argparse
...
#-------------------------------- Duration targets ------------------------------------------------------
    subset = librispeech_md_file.iloc[0]['subset']

    if 'tr' in subset or 'train' in subset:
        target_samples = 30 * 3600 * RATE   # 30 hours
    elif 'cv' in subset or 'dev' in subset or 'val' in subset:
        target_samples = 4 * 3600 * RATE    # 4 hours
    elif 'tt' in subset or 'test' in subset:
        target_samples = 4 * 3600 * RATE    # 4 hours
    else:
        target_samples = None
...
#------------------------------------------------------------------------------------------------------------        
        # Compute MIN duration (correct for min mode)
        # Get true speech duration from metadata (NOT padded audio)
        sources = [librispeech_md_file.iloc[pair[i]] for i in range(n_src)]
        speech_lengths = [source['length'] for source in sources]

        # For min mode use shortest source
        current_length = min(speech_lengths)

        if target_samples is not None:
            if total_samples + current_length > target_samples:
                print(f"Stopping metadata at target duration for {subset}")
                break

        total_samples += current_length
#-------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------
    if target_samples:
        ...
        print(f"Final duration for {subset}: {hours:.2f} hours")
#-------------------------------------------------------------------------------------------------------
...
#     # Check the subset name from the metadata
#     subset = librispeech_md_file.iloc[0]['subset']
    
#     # --- FIX: Set Target counts based on subset name ---
#     if 'tr' in subset or 'train' in subset:
#         target = None  # Exhaustive (One pass)
#     elif 'cv' in subset or 'val' in subset or 'dev' in subset:
#         target = 500   # Set your desired Validation count
#     elif 'tt' in subset or 'test' in subset:
#         target = 1000  # Set your desired Test count
#     else:
#         target = 3000  # Fallback
        
#     print(f"Generating {target if target else 'all'} pairs for subset: {subset}")
...
#     # Otherwise we want specific number of mixtures
#     else:
#         # Loop until we reach the target
#         while len(utt_pairs) < target:
#             utt_pairs = set_utt_pairs(librispeech_md_file, utt_pairs, n_src)
#             noise_pairs = set_noise_pairs(utt_pairs, noise_pairs,
#                                           librispeech_md_file, wham_md_file)
            
#             # Use the FAST deduplication logic
#             utt_pairs, noise_pairs = remove_duplicates(utt_pairs, noise_pairs)
            
#             # Safety break: If we have enough unique pairs, stop generating
#             print(f"   Current unique pairs: {len(utt_pairs)} / {target}")
            
#         utt_pairs = utt_pairs[:target]
#         noise_pairs = noise_pairs[:target]
...
def set_pairs(librispeech_md_file, wham_md_file, n_src):
    ...
    noise_pairs = []
    # In train sets utterance are only used once
    if 'tr' in librispeech_md_file.iloc[0]['subset']:
        utt_pairs = set_utt_pairs(librispeech_md_file, utt_pairs, n_src)
        noise_pairs = set_noise_pairs(utt_pairs, noise_pairs,
                                      librispeech_md_file, wham_md_file)
