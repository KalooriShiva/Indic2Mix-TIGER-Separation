...
# CLIENT_ID = "tOWQrugxhteHcvwcdtAi" 
# CLIENT_SECRET = "alcpbtdb4K6lFlqzLRE1HvanRnTGT2EMjFYHIiKi" 
...
# # Limits for CV (Validation) and TT (Test) splits
# CV_TT_LIMIT = 3000 
...
# def get_count(split):
#     """Utility to count how many files are already in a split folder."""
#     return len(os.listdir(os.path.join(BASE_OUTPUT_DIR, split)))
...
#             token = json.load(f)
#             # Basic check if the file we loaded is an error response
#             if 'access_token' not in token:
#                 print("[INFO] Saved token file is invalid. Re-authenticating...")
#                 token = None

#     if not token:
#         oauth = OAuth2Session(CLIENT_ID)
#         auth_url, _ = oauth.authorization_url("https://freesound.org/apiv2/oauth2/authorize/")
#         print(f"\n[ACTION] Go to this URL to authorize the app:\n{auth_url}")
#         code = input("\nEnter the code displayed in your browser: ").strip()
        
#         # Exchange authorization code for access token
#         response = requests.post("https://freesound.org/apiv2/oauth2/access_token/",
#             data={
#                 "client_id": CLIENT_ID, 
...
#     # auth_type must be "oauth" for downloading original files
#     fs_client.set_token(token['access_token'], "oauth")
#     return fs_client

# # --- 4. PROCESSING LOGIC ---
# def process_and_save(file_path, split_name, meter, current_limit=None):
...
#             # Stop if we hit split limit
#             if current_limit and get_count(split_name) >= current_limit:
#                 break
...
#     splits = [
#         ('tt', tt_files, CV_TT_LIMIT), 
#         ('cv', cv_files, CV_TT_LIMIT), 
#         ('tr', tr_files, None)
#     ]

#     for split_name, sound_list, limit in splits:
#         print(f"\n--- Processing {split_name.upper()} split ({len(sound_list)} source files) ---")
        
#         for sound in sound_list:
#             # Check if split is already full
#             if limit and get_count(split_name) >= limit:
#                 print(f"Target reached for {split_name}")
#                 break
...
#                 # Process all files currently in the input dir (should be just the one we downloaded)
#                 for f in os.listdir(INPUT_DIR):
#                     full_path = os.path.join(INPUT_DIR, f)
#                     saved = process_and_save(full_path, split_name, meter, limit)
...
MY_API_KEY = "akpbtdb4K6lFkqzLRE1HvanRnTGT2EMjFYHliKi" 
